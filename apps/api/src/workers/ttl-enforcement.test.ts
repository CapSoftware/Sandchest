...
describe('ttl-enforcement worker', () => {
  ...
  test('does not stop sandboxes within TTL', async () => {
    ...
    // assignNode sets status to 'running' and startedAt to now — within TTL
  ...
  test('ignores non-running sandboxes with expired TTL', async () => {
    const id = generateUUIDv7()
    await Effect.runPromise(
      sandboxRepo.create({
        id,
    ...
    // Leave as 'queued' — findExpiredTtl only returns running sandboxes

    const count = await run(runWorkerTick(ttlEnforcementWorker, 'inst-1'))
    expect(count).toBe(0)
  })

  test('stops a running sandbox whose TTL has expired', async () => {
    const id = generateUUIDv7()
    await Effect.runPromise(
      sandboxRepo.create({
        id,
    ...
    // assignNode sets status to 'running' and startedAt to now
    await Effect.runPromise(sandboxRepo.assignNode(id, 'org_test', generateUUIDv7()))

    // Backdate startedAt so TTL is already expired
    const row = await Effect.runPromise(sandboxRepo.findById(id, 'org_test'))
    ;(row as { startedAt: Date }).startedAt = new Date(Date.now() - 10_000)
    ...
    expect(updated!.failureReason).toBe('ttl_exceeded')
    expect(updated!.endedAt).toBeInstanceOf(Date)
  })

  test('stops multiple expired sandboxes in one tick', async () => {
  ...
  test('releases slot when sandbox has a slotIndex', async () => {
    ...
    await Effect.runPromise(sandboxRepo.assignNode(id, 'org_test', nodeId, 3))
