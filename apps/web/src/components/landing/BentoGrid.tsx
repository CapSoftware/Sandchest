...
function BentoCell({ feature }: { feature: typeof features[number] }) {
  const containerRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    const el = containerRef.current
    if (!el) return
    const init = animationInits[feature.animationId]
    if (!init) return
    return init(el)
  }, [feature.animationId])
  ...
  return (
    <div className="bento-cell">
      ...
      <div className="bento-content">
        <h4 className="bento-title">{feature.title}</h4>
    ...
    <section id="features" className="section">
      ...
      <div className="bento-grid">
        {features.map((feature) => (
          <BentoCell key={feature.animationId} feature={feature} />
        ))}
      </div>
