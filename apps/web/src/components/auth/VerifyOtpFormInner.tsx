...
const OTP_LENGTH = 6
const DIGIT_KEYS = Array.from({ length: OTP_LENGTH }, (_, i) => ({ key: `d${i}`, pos: i }))
        ...
        <div className="otp-inputs">
          {DIGIT_KEYS.map((slot) => (
            <input
              key={slot.key}
              ref={(el) => {
                inputRefs.current[slot.pos] = el
              }}
              type="text"
              inputMode="numeric"
              maxLength={1}
              value={digits[slot.pos]}
              onChange={(e) => handleChange(slot.pos, e.target.value)}
              onKeyDown={(e) => handleKeyDown(slot.pos, e)}
              onPaste={slot.pos === 0 ? handlePaste : undefined}
              className="otp-digit"
              disabled={loading}
              aria-label={`Digit ${slot.pos + 1}`}
            />
          ))}
        </div>
